...
function create_manual_right_click(event) {
    if (event.player.mainHandItem == "create:create_manual") {
        ...
        if (ponder == null) {
            event.player.tell(
                'The file "kubejs/ponder.json" does not exist on the server side, move it from the client to the server files so that create_manual works properly.'
            )
        ...
        if (!event.player.stages.has("notified_about_create_manual_options")) {
            ...
            event.player.tell(
                "Shift + scroll if you want to change screen mode or click on the block from Create for which you want to get help."
            )
        }
...
NetworkEvents.dataReceived("create_manual_change_screen", (event) => {
    ...
    if (
        ...
        event.player.persistentData.create_manual_page_type == "PonderTagIndexScreen"
    ) {
